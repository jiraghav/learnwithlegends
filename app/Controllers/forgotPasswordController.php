...
class forgotPasswordController extends controller
{
    ...
    {
        $this->view('auth/change_password', ['email' => $email]);
    }
    ...
    public function confirm_reset($email, $token)
    {
        ...
        $expires_at = strtotime(date("Y-m-d H:i:s", strtotime("{$password_reset->created_at} + 30 minute")));
        if ($expires_at < time()) {
            $password_reset->delete();
            Session::putFlash("danger", "This link has expired.");
            Redirect::to('forgot-password');
        }
