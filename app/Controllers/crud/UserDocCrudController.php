...
class UserDocCrudController extends controller
{
    ...
    public function push_to_state()
    {
        ...
        if (($doc == null) || (!file_exists($doc->path)  && $doc->isDocument())) {
    ...
    public function update_links()
    {
        ...
        $validator->check(Input::all(), [
            "links.webtalk" => [
                "name" => "Webtalk profile link",
                // "required" => true,
                "url" => true,
            ],
            "links.viiral" => [
                "name" => "Viiral profile link",
                "required" => true,
                "url" => true,
            ],
            "links.unifying" => [
                "name" => "Unifying profile link",
                "required" => true,
                "url" => true,
            ],
        ]);
        ...
        $last_doc = UserDocument::where('user_id', $auth->id)
            ->where('document_type', 3) //links
            ->latest()
            ->first();
        ...
        if ($last_doc != null) {
            if ($last_doc->is_status(1) || $last_doc->is_status(2)) {
                Session::putFlash("danger", "Links is in review or approved.");
                Redirect::back();
            }
        }
