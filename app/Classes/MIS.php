...
class MIS
{
	...
	public static	function format_to_thousand_unit($input)
	{
	...
	public static function buildView($view, $data = [], $multiple = false, $light = false)
	{
		$controller = new \controller;
		return $controller->buildView($view, $data, $multiple, $light);
	}

	public static function filter_note($showing, $of, $total_set, $sieve, $pass_mark = 1, $full = false)
	{
		...
		if ($full == true) {
			$response = compact("note", 'showing', 'of', 'total_set');
			return $response;
		}
		return $note;
	}
	...
	public static function generate_form($data, $action, $button, $function = '', $require_confirmation = false)
	{
		...
		if ($require_confirmation == true) {
			$btn =  '<button type="button" onclick="$confirm_dialog = new DialogJS(submit_form, [this])"  class="btn btn-sm btn-outline-dark">' . $button . '</button>';
		} else {

			$btn =  '<button type="submit"   class="btn btn-sm btn-outline-secondary">' . $button . '</button>';
		}
    ...
    <form style="display: inline;"  $function_attribute class="ajax_form" method="post" action="$action">
    $inputs  
    $btn
    </form>
