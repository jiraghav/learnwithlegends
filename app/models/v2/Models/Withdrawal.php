...
class Withdrawal extends Eloquent
{
	...
	public static function payoutBalanceFor($user_id)
	{
		...
		$commission_balance =  Wallet::TraitavailableBalanceOnUser($user_id, 'commission');
		$deposit_balance    =  Wallet::TraitavailableBalanceOnUser($user_id, 'deposit');

		$investment_credits    =  Wallet::TraitavailableBalanceOnUser($user_id, 'investment');


		$total_earnings =   $commission_balance + $investment_credits;
		...
		$payout_balance = $payout_wallet;

		$payout_book_balance = Wallet::bookBalanceOnUser($user_id);
