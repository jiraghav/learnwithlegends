...
class SupportTicket extends Eloquent
{
	...
	{
		$recipients = User::whereRaw("id in ($this->user_id)")
			->get()
			->keyBy('email')->map(function ($item) {
	...
	public function compile_email($core_msg)
	{
		...
		return MIS::compile_email("$core_msg");
	}
	...
	{
		$link = Config::domain() . "/support/close_ticket?ticket_code={$this->code}";
		return $link;
	}
	...
	public function getcloseButtonAttribute()
	{
		...
		href='javascript:void(0);' class='btn btn-sm btn-outline-danger'>Close</a>";
