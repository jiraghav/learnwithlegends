function Cart() {
    ...
    this.add_item = function($item, $is_buy = false) {
        ...
        if (this.contains_object($item, this.$items)) {
            window.show_notification('<b>' + $item.market_details.name + '</b><br> Already in Cart! <br> <a class="btn btn-block btn-rounded btn-outline-secondary btn-sm" href=' + $checkout_url + '>Check out</a>');
            return;
        }
        ...
        window.show_notification('<b>' + $item.market_details.name + '</b><br> Added to cart successfully! <br> <a class="btn btn-block btn-rounded btn-outline-secondary btn-sm" href=' + $checkout_url + '>Check out</a>');
...
function Shop($sce) {
    ...
    this.fetch_products = function() {
        ...
        $this = this;
        $url = `${$base_url}/shop/fetch_products`


        let sieveParams = Object.fromEntries(
